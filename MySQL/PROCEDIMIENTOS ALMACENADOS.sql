...
DELIMITER $
DROP PROCEDURE IF EXISTS `PAGINA_WEB`.`SP_AgregarUbicacionProfecional` $ -- facilita actualizar el procedimiento
CREATE PROCEDURE `PAGINA_WEB`.`SP_AgregarUbicacionProfecional`(in _ID_USUARIO int, 
...
DELIMITER $
DROP PROCEDURE IF EXISTS `PAGINA_WEB`.`SP_EliminarUbicacionProfecional` $ -- facilita actualizar el procedimiento
CREATE PROCEDURE `PAGINA_WEB`.`SP_EliminarUbicacionProfecional`(in _ID_USUARIO int, 
																		  in _ID_UBICACION int,
																		  out _msj varchar(100))
BEGIN 
