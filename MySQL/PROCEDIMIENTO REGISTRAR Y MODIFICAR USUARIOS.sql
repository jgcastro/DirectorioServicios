...
DELIMITER $
DROP PROCEDURE IF EXISTS `DIRECTORIO_SERVICIOS`.`SP_REGISTRAR_O_ACTUALIZAR_USUARIO` $ -- facilita actualizar el procedimiento
CREATE PROCEDURE `DIRECTORIO_SERVICIOS`.`SP_REGISTRAR_O_ACTUALIZAR_USUARIO`(IN _ID_USUARIOS INT,
...
DROP PROCEDURE IF EXISTS `DIRECTORIO_SERVICIOS`.`SP_EliminarUsuario` $ -- facilita actualizar el procedimiento
CREATE PROCEDURE `DIRECTORIO_SERVICIOS`.`SP_EliminarUsuario`(in _id integer, out msj varchar(100))
BEGIN
